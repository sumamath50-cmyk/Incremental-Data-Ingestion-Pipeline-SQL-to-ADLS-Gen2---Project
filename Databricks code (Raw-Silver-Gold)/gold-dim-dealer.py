...
if spark.catalog.tableExists('cars_catelog.gold.dim_dealer'):
    ...
    df_sink1 = spark.sql('''
        ...
        FROM cars_catelog.gold.dim_dealer
    ''')
...
df_filter = df_src.join(df_sink1, df_src['Dealer_ID'] == df_sink1['Dealer_ID'], "left").select(df_src['Dealer_ID'], df_src['DealerName'], df_sink1['dim_dealer_key'])
...
if (incremental_flag == '0'):
    ...
    max_value_df = spark.sql("select max(dim_dealer_key) from cars_catelog.gold.dim_dealer")
...
# Incremental run
if spark.catalog.tableExists('cars_catelog.gold.dim_dealer'):
    delta_tbl = DeltaTable.forPath(spark, "abfss://gold@datalakecar.dfs.core.windows.net/dim_dealer")
...
#Initial run
    ...
    df_final.write.format("delta")\
        .mode('overwrite')\
            .option("path","abfss://gold@datalakecar.dfs.core.windows.net/dim_dealer")\
                .saveAsTable("cars_catelog.gold.dim_dealer")

# COMMAND ----------

# MAGIC %sql
# MAGIC select * from cars_catelog.gold.dim_dealer