...
if spark.catalog.tableExists('cars_catelog.gold.dim_date'):
    ...
    df_sink1 = spark.sql('''
        ...
        FROM cars_catelog.gold.dim_date
    ''')
...
df_filter = df_src.join(df_sink1, df_src['Date_ID'] == df_sink1['Date_ID'], "left").select(df_src['Date_ID'], df_sink1['dim_date_key'])
...
if (incremental_flag == '0'):
    ...
    max_value_df = spark.sql("select max(dim_date_key) from cars_catelog.gold.dim_date")
...
# Incremental run
if spark.catalog.tableExists('cars_catelog.gold.dim_date'):
    delta_tbl = DeltaTable.forPath(spark, "abfss://gold@datalakecar.dfs.core.windows.net/dim_date")
...
#Initial run
    ...
    df_final.write.format("delta")\
        .mode('overwrite')\
            .option("path","abfss://gold@datalakecar.dfs.core.windows.net/dim_date")\
                .saveAsTable("cars_catelog.gold.dim_date")

# COMMAND ----------

# MAGIC %sql
# MAGIC select * from cars_catelog.gold.dim_date